...
class Astatine(object):
    ...
    def __init__(self, host='localhost', port=8080, debug=True, reload=False, server=None, quiet=None, sql_name=None):
        ...
        self._static_files_ext = ['css', 'scss', 'less', 'png', 'jpg', 'jpeg', 'gif', 'tiff',
                                  'psd', 'raw', 'svg', 'ico', 'js', 'otf', 'ttf', 'eot', 'woff', 'woff2', 'mp4', 'mov', 'mkv']
    ...
    def generate_uid(self, table_name, id_name, length=20):
        ...
        off_length = length if self.uid_length is length else self.uid_length
        while not is_unique:
...
class AstatineSMTP(object):
    ...
    def send_email(self, receiver, subject, dkim_path=None, dkim_selector=None, text=None, html=None):
        message = MIMEMultipart('alternative')
        message['From'] = self.sender
        message['To'] = receiver
        message['Subject'] = subject
        ...
        if dkim_path:
            private_key = open(dkim_path).read()
            signature = dkim.sign(message.as_bytes(), str.encode(dkim_selector), b'tygzy.net', private_key.encode(),
